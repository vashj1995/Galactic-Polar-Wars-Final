...
Game::Game() : m_pWindow(nullptr), m_bRunning(true), m_frames(0), m_currentScene(nullptr), m_currentSceneState(NO_SCENE)
{
...
void Game::start()
{
	m_musicVolumeControl = 25;
	m_soundVolumeControl = 35;
...
void Game::changeSceneState(const SceneState new_state)
{
	if (new_state != m_currentSceneState) {
		...
		case STORY_SCENE:
			...
			std::cout << "story scene activated" << std::endl;
			break;
		case LEVEL_ONE:
			m_currentScene = new LevelOne();
			std::cout << "Level One activated" << std::endl;
			break;
		case LEVEL_TWO:
			m_currentScene = new LevelTwo();
			std::cout << "Level Two activated" << std::endl;
			break;
		case BOSS_LEVEL:
			m_currentScene = new BossLevel();
			std::cout << "Boss Level activated" << std::endl;
			break;
		case PAUSE_SCENE:
			m_currentScene = new PauseScene();
			std::cout << "pause scene activated" << std::endl;
			break;
		case END_SCENE:
...
{
	m_currentScene->update(); //BUG

	SoundManager::Instance().setMusicVolume(m_musicVolumeControl);
	SoundManager::Instance().setSoundVolume(m_soundVolumeControl);
}
...
void Game::handleEvents()
{
	...
	//VOLUME CONTROL
	if (EventManager::Instance().isKeyDown(SDL_SCANCODE_KP_PLUS))
	{
		m_musicVolumeControl++;
	}

	if (EventManager::Instance().isKeyDown(SDL_SCANCODE_KP_MINUS))
	{
		m_musicVolumeControl--;
	}
