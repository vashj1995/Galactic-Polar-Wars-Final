	...
	if (!m_reachedDoor && !m_gameOver)
	{
		...
		if (m_pEnemy.size() > 0 && m_pPlayerBullets.size() > 0)
		{
			for (unsigned i = 0; i < m_pEnemy.size(); i++)
			{
				...
				{
					if (CollisionManager::circleAABBCheck(m_pPlayerBullets[j], m_pEnemy[i]) == true && m_pEnemy[i]->isEnabled() == true)
					{
...
void LevelTwo::clean()
{
	SoundManager::Instance().unload("D&G", SOUND_MUSIC);
...
void LevelTwo::start()
{
	...
	// Music:
	SoundManager::Instance().load("../Assets/audio/D&G.mp3", "D&G", SOUND_MUSIC);
	SoundManager::Instance().playMusic("D&G");
	...
	m_pPlayer->getTransform()->position = glm::vec2(82.0f, 1910.0f);
	addChild(m_pPlayer, 4);
	// End of Player -----------

	// Create new Level & add platforms(tiles).
	m_pEngineLevel = new Level(20, 50, ENGINE);
	m_pEngineLevel->addTiles(m_pTiles, m_pDecorations);
	addChild(m_pEngineLevel, 8);
	...
	m_pEnemy.push_back(new Enemy(glm::vec2(140.0f, 890.0f), Enemy::ALIEN));
	m_pEnemy.push_back(new Enemy(glm::vec2(140.0f, 1530.0f), Enemy::ALIEN));
	m_pEnemy.push_back(new Enemy(glm::vec2(582.0f, 450.0f), Enemy::ALIEN));
	m_pEnemy.push_back(new Enemy(glm::vec2(331.0f, 330.0f), Enemy::ALIEN));
	m_pEnemy.push_back(new Enemy(glm::vec2(82.0f, 1170.0f), Enemy::ALIEN));
	m_pEnemy.push_back(new Enemy(glm::vec2(696.0f, 1650.0f), Enemy::ALIEN));
	...
	m_pFish[0]->getTransform()->position = glm::vec2(636.0f, 950.0f);
	
	m_pFish.push_back(new Fish());
	m_pFish[1]->getTransform()->position = glm::vec2(252.0f, 1550.0f);
	
	m_pFish.push_back(new Fish());
	m_pFish[2]->getTransform()->position = glm::vec2(61.5466f, 1350.0f);
	
	m_pFish.push_back(new Fish());
	m_pFish[3]->getTransform()->position = glm::vec2(251.63f, 670.0f);

	m_pFish.push_back(new Fish());
	m_pFish[4]->getTransform()->position = glm::vec2(99.0f, 110.0f);

	m_pFish.push_back(new Fish());
	m_pFish[5]->getTransform()->position = glm::vec2(152.0f, 510.0f);
	...
	m_pAlienBlaster->getTransform()->position = glm::vec2(548.0f, 1710.0f);
...
// IMGUI --------------------------------
void LevelTwo::GUI_Function() const
{
