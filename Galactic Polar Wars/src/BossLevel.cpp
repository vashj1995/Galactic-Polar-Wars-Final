...
void BossLevel::update()
{
	...
	if (!m_gameOver && !m_gameWon)
	{
		...
		{
			std::cout << "\nStart loop";
			SoundManager::Instance().playMusic("BFLoop", -1);
		}
		...
		if (m_gregoryHitRecently)
		{
			++m_gregoryAngryTimer;
			if (m_gregoryAngryTimer >= 300)
			{
				m_gregoryAngryTimer = 0;
				m_gregoryHitRecently = false;
				m_pGregory->flipAngry();
			}
		}
		...
		{
			if (CollisionManager::AABBCheck(m_pPlayer, m_pTiles[i])) //BUG
			{
		...
		if (m_pEnemy.size() > 0 && m_pPlayerBullets.size() > 0)
		{
			for (unsigned i = 0; i < m_pEnemy.size(); i++)
			{
				...
				{
					if (CollisionManager::circleAABBCheck(m_pPlayerBullets[j], m_pEnemy[i]) == true && !m_pEnemy[i]->isDead() && m_pEnemy[i]->isEnabled() == true)
					{
		...
		// Check collision between Pigoo bullets & Gregory.
		// Delete bullet upon collision & update Gregory's health.
		if (m_pPlayerBullets.size() > 0 && !m_pGregory->isDead())
		{
			for (unsigned j = 0; j < m_pPlayerBullets.size(); j++)
			{
				if (CollisionManager::circleAABBCheck(m_pPlayerBullets[j], m_pGregory) == true)
				{
					SoundManager::Instance().playSound("GHit");

					m_gregoryHitRecently = true;
					m_pGregory->flipAngry();

					//if (m_gregoryShootTimer <)
					m_gregoryShootTimer = 70; //So you can't stuff him full of bullets and easily get away


					m_pGregory->getHit();
					m_pGregory->setHealthPoints(false);
					m_pGregHealth->setHealthPointsUI(m_pGregory->getHealthPoints());
					if (m_pGregory->getHealthPoints() == 0)
					{
						SoundManager::Instance().playSound("GDeath");

						m_pGregory->setIsDead();
					}
					removeChild(m_pPlayerBullets[j]);
					m_pPlayerBullets[j] = nullptr;
					m_pPlayerBullets.erase(m_pPlayerBullets.begin() + j);
		...
		if (m_pGregory->isDeathTimerDone())
		{
			m_gameWon = true;
		}
		...
		if (!m_pEnemy.empty() && !m_invincibleTimer)
		{
			for (unsigned i = 0; i < m_pEnemy.size(); i++)
			{
				if (CollisionManager::circleAABBCheck(m_pPlayer, m_pEnemy[i]) == true)
				{
					if (m_gotHitRecently == false)
					{
						SoundManager::Instance().playSound("PHit");
		...
		// Check collision between Gregory & Pigoo and decrement HP.
		if (!m_invincibleTimer)
		{
			if (CollisionManager::circleAABBCheck(m_pPlayer, m_pGregory) == true)
			{
				if (m_gotHitRecently == false)
				{
					SoundManager::Instance().playSound("PHit");
		...
		// Collision between Pigoo & Gregory bullets and decrement HP.
		if (!m_pGregoryBullets.empty() && !m_invincibleTimer)
		{
			for (unsigned i = 0; i < m_pGregoryBullets.size(); i++)
			{
				if (CollisionManager::circleAABBCheck(m_pGregoryBullets[i], m_pPlayer) == true)
				{
					if (m_gotHitRecently == false)
					{
						SoundManager::Instance().playSound("PHit");
					...
					m_gotHitRecently = true;
					// Delete bullet if collision happens.
					removeChild(m_pGregoryBullets[i]);
					m_pGregoryBullets[i] = nullptr;
					m_pGregoryBullets.erase(m_pGregoryBullets.begin() + i);
					m_pGregoryBullets.shrink_to_fit();
				}
			}
		}

		//Check Collision between Enemy Bullets and Boxes
		if (m_pTiles.size() > 0 && m_pGregoryBullets.size() > 0 && m_pGregory->isDead() == false)
		{
			for (unsigned i = 0; i < m_pTiles.size(); i++)
			{
				for (unsigned j = 0; j < m_pGregoryBullets.size(); j++)
				{
					if (CollisionManager::circleAABBCheck(m_pGregoryBullets[j], m_pTiles[i]) == true)
					{
						removeChild(m_pGregoryBullets[j]);
						m_pGregoryBullets[j] = nullptr;
						m_pGregoryBullets.erase(m_pGregoryBullets.begin() + j);
						m_pGregoryBullets.shrink_to_fit();
					}
				}
			}
		...
		// GREGORY BULLETS & TURNING TO FACE PIGOO ----------------------------------
		if (!m_pGregory->isDead() && !m_pGregory->getIsAnimating())
		{
			m_pGregory->moveToTarget(m_pPlayer);
			bool direction;

			if (Util::distance(m_pPlayer->getTransform()->position, m_pGregory->getTransform()->position) < 500.0f || m_gregoryHitRecently)
			{
				// Turn to face Pigoo in a certain range.
				if (m_pPlayer->getTransform()->position.x < m_pGregory->getTransform()->position.x)
				{
					direction = false;
					//Keeps moving if got hit.
					if (!m_gotHitRecently)
						m_pGregory->setAnimationState(GREGORY_IDLE_LEFT);
				}
				else
				{
					direction = true;
					if (!m_gotHitRecently)
						m_pGregory->setAnimationState(GREGORY_IDLE_RIGHT);
				}
				// Increment the timer while Pigoo is in range.
				++m_gregoryShootTimer;
				// Gregory can only shoot every 140 frames.
				// The >= is for if Pigoo goes out of range.
				if (m_gregoryShootTimer >= 70)
				{
					m_gregoryShootTimer = 0;
					// Shoot if Pigoo is at the same level (ish).
					if (m_pPlayer->getTransform()->position.y >= m_pGregory->getTransform()->position.y - m_pGregory->getHeight() && m_pPlayer->getTransform()->position.y <= m_pGregory->getTransform()->position.y + m_pGregory->getHeight())
					{
						if (m_pGregory->isFacingRight() == true)
						{
							// New bullet in Pigoo's direction.
							SoundManager::Instance().playSound("poof");
							m_pGregoryBullets.push_back(new GregoryBullet(glm::vec2(m_pGregory->getTransform()->position.x + 60, m_pGregory->getTransform()->position.y + 10), direction, HORIZONTAL));

							if (m_gregoryHitRecently)
							{
								m_pGregoryBullets.push_back(new GregoryBullet(glm::vec2(m_pGregory->getTransform()->position.x + 60, m_pGregory->getTransform()->position.y + 10), direction, DIAGONAL_UP));
								m_pGregoryBullets.push_back(new GregoryBullet(glm::vec2(m_pGregory->getTransform()->position.x + 60, m_pGregory->getTransform()->position.y + 10), direction, DIAGONAL_DOWN));
							}

							// AddChild the bullets so they will be drawn.
							for (unsigned i = 0; i < m_pGregoryBullets.size(); i++)
							{
								addChild(m_pGregoryBullets[i], 3);
							}
						}
						else
						{
							// New bullet in Pigoo's direction.
							SoundManager::Instance().playSound("poof");
							m_pGregoryBullets.push_back(new GregoryBullet(glm::vec2(m_pGregory->getTransform()->position.x - 80, m_pGregory->getTransform()->position.y + 10), direction, HORIZONTAL));

							if (m_gregoryHitRecently)
							{
								m_pGregoryBullets.push_back(new GregoryBullet(glm::vec2(m_pGregory->getTransform()->position.x - 80, m_pGregory->getTransform()->position.y + 10), direction, DIAGONAL_UP));
								m_pGregoryBullets.push_back(new GregoryBullet(glm::vec2(m_pGregory->getTransform()->position.x - 80, m_pGregory->getTransform()->position.y + 10), direction, DIAGONAL_DOWN));
							}
		...
		// Delete Gregory bullets if they go off screen.
		if (!m_pGregoryBullets.empty())
		{
			for (unsigned i = 0; i < m_pGregoryBullets.size(); i++)
			{
				if (m_pGregoryBullets[i]->getTransform()->position.x < (m_pPlayer->getTransform()->draw_position.x - Config::SCREEN_WIDTH / 2) || m_pGregoryBullets[i]->getTransform()->draw_position.x >(m_pPlayer->getTransform()->position.x + Config::SCREEN_WIDTH / 2))
				{
					removeChild(m_pGregoryBullets[i]);
					m_pGregoryBullets[i] = nullptr;
					m_pGregoryBullets.erase(m_pGregoryBullets.begin() + i);
...
void BossLevel::clean()
{
	removeAllChildren();
}

void BossLevel::handleEvents()
{
	...
	if (EventManager::Instance().getMouseButton(0))
	{
		...
		if (!m_pGregory->isDead() && !m_pGregory->getIsAnimating())
		{
			if (Util::distance(m_pPlayer->getTransform()->position, m_pGregory->getTransform()->position) < 120)
			{
				m_pGregory->getHit();
				m_pGregory->setHealthPoints(false);
				m_pGregHealth->setHealthPointsUI(m_pGregory->getHealthPoints());
...
void BossLevel::start()
{
	...
	// Music:
	SoundManager::Instance().load("../Assets/audio/Gregory_Death.wav", "GDeath", SOUND_SFX);
	SoundManager::Instance().load("../Assets/audio/Temp_Gregory_Hit.wav", "GHit", SOUND_SFX);
	SoundManager::Instance().load("../Assets/audio/poof.wav", "poof", SOUND_SFX);

	SoundManager::Instance().load("../Assets/audio/BossFight_new_rise.mp3", "BFRise", SOUND_MUSIC);
	SoundManager::Instance().load("../Assets/audio/BossFight_new_loop.mp3", "BFLoop", SOUND_MUSIC);

	SoundManager::Instance().playMusic("BFRise", 0);

	// Set GUI Title
	m_guiTitle = "Boss Level";

	// Player ------------------
	m_pPlayer = new Player();
	m_pPlayer->setFacingRight(true);
	addChild(m_pPlayer, 4);
	m_pPlayer->getTransform()->position = glm::vec2(62.0f,630.0f);
	m_pPlayer->setAlienBlaster(true);
	// End of Player -----------

	//Enemies ------------------
	//m_pEnemy.push_back(new Enemy(glm::vec2(408.0f, 287.0f), Enemy::ALIEN)); //Left platform
	m_pEnemy.push_back(new Enemy(glm::vec2(1864.0f, 335.0f), Enemy::ALIEN));//Far Right Up
	m_pEnemy.push_back(new Enemy(glm::vec2(1822.0f, 617.0f), Enemy::ALIEN));//Far Right Down
	m_pEnemy.shrink_to_fit();
	...
	// Boss --------------------
	m_pGregory = new Gregory(glm::vec2 ( 1200.0f, 592.0f));
	addChild(m_pGregory, 4);

	// Create new Level & add platforms(tiles).
	m_pBossLevel = new Level(51, 20, BOSS);
	m_pBossLevel->addTiles(m_pTiles, m_pDecorations);
	addChild(m_pBossLevel, 8);
	...
	m_pFish[0]->getTransform()->position = glm::vec2(116.0f, 432.0f); //Platform above the playerSpawn
	m_pFish.push_back(new Fish());
	m_pFish[1]->getTransform()->position = glm::vec2(1164.0f, 180.0f); //Middle (between platforms) one
	m_pFish.push_back(new Fish());
	m_pFish[2]->getTransform()->position = glm::vec2(1456.0f, 105.0f); //Middle-Left one
	m_pFish.push_back(new Fish());
	m_pFish[3]->getTransform()->position = glm::vec2(854.0f, 141.0f); //Middle-right one
	m_pFish.push_back(new Fish());
	m_pFish[4]->getTransform()->position = glm::vec2(96.0f, 105.0f); //Far left one
	...
	for (int i = 0; i <= 75; i++)
	{
		...
		if (altFloor % 2 == 0)
		{
			m_pFloorTiles.push_back(new Floor(glm::vec2((40 * i) + 20, 1000.0f), FF1));
		}
		// Else, odd, FloorType 2:
		else
		{
			m_pFloorTiles.push_back(new Floor(glm::vec2((40 * i) + 20, 1000.0f), FF2));
		}
		// AddChild the Floor.
	...
	// Background--------------------------
	m_pEngineBackground = new EngineBackground();
	addChild(m_pEngineBackground, 1);

	m_pBossDecos.push_back(new BossDecoration(BD_CONSOLE, glm::vec2(1200.0f, 530.0f)));
	addChild(m_pBossDecos[0], 3);
	m_pBossDecos.push_back(new BossDecoration(BD_WINDOW, glm::vec2(780.0f, 200.0f)));
	addChild(m_pBossDecos[1], 2);
	...
	m_pGregHealth = new GregoryHealthBar();
	m_pGregHealth->getTransform()->position = glm::vec2(1190.0f, 40.0f);
	addChild(m_pGregHealth, 5);
	...
	m_pWeaponUI.push_back(new WeaponUI(RMB_ALIEN_BLASTER, glm::vec2(60.0f + m_pWeaponUI[0]->getWidth() + 20, m_pHPUI->getTransform()->position.y + m_pHPUI->getHeight() + 20)));
...
// IMGUI --------------------------------
void BossLevel::GUI_Function() const
{
