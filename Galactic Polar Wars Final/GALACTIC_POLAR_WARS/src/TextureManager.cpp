...
bool TextureManager::tileMapExists(const std::string& tile_map_name)
{
	return m_tileMapMap.find(tile_map_name) != m_tileMapMap.end();
}
	...
	const std::string& sprite_sheet_name)
{
	...
	auto spriteSheet = new SpriteSheet(sprite_sheet_name); ///!!!!!!BUG
...
// Loads in the key, x, y, w, h, and type of tile from the .txt file and associates it with the .png.
bool TextureManager::loadTileMap(const std::string& data_file_name,
...

{
	...
	// create a new spritesheet object and allocate memory for it
	auto tileMap = new TileMap(tile_map_name);
	...
	if (dataFile)
	{
		...
		while (std::getline(dataFile, inputLine))
		{
			...
			tile.key = tokens[0];
			tile.x = std::stoi(tokens[1]);
			tile.y = std::stoi(tokens[2]);
			tile.w = std::stoi(tokens[3]);
			tile.h = std::stoi(tokens[4]);
			tile.t = std::stoi(tokens[5]);
	...
	// load the sprite texture and store it in the textureMap
	load(texture_file_name, tile_map_name);

	// get a pointer to the Texture and store it in the new tilemap object
	tileMap->setTexture(getTexture(tile_map_name));

	// store the new tilemap in the tileMap
	m_tileMapMap[tile_map_name] = tileMap;
...
// Draws the tiles from the tilemap.
void TextureManager::drawTile(const std::string tile_map_id, const std::string& id, const int x, const int y, const double angle, const int alpha, const bool centered, const SDL_RendererFlip flip)
{
	SDL_Rect srcRect;
	SDL_Rect destRect;
	...
	srcRect.x = m_tileMapMap[tile_map_id]->getTile(id).x;
	srcRect.y = m_tileMapMap[tile_map_id]->getTile(id).y;
	textureWidth = srcRect.w = m_tileMapMap[tile_map_id]->getTile(id).w;
	textureHeight = srcRect.h = m_tileMapMap[tile_map_id]->getTile(id).h;
	...
	SDL_SetTextureAlphaMod(m_textureMap[tile_map_id].get(), alpha);
	SDL_RenderCopyEx(Renderer::Instance()->getRenderer(), m_textureMap[tile_map_id].get(), &srcRect, &destRect, angle, nullptr, flip);
}
...
TileMap* TextureManager::getTileMap(const std::string& name)
{
	return m_tileMapMap[name];
}
