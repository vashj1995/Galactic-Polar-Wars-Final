...
void Scene::updateDrawPositions()
{
	...
	for (auto& objects : m_displayList) 
	{
		if (objects != nullptr)
		{
			...
			//	break;
			objects->getTransform()->draw_position = objects->getTransform()->position	- this->getTransform()->position;
		}
	}
}

// WIP SOPH (for pauseState)

//void Scene::PushState(Scene* pState) // Invoked going to PauseState from GameState
//{
//	s_states.push_back(pState);
//	s_states.back()->Enter();
//}
//
//void Scene::PopState() // Invoked going back to GameState from PauseState
//{
