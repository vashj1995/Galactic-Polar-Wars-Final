...
public:
	...
	bool loadTileMap(const std::string& data_file_name, const char* texture_file_name, const std::string& tile_map_name);
	...
	// For drawing the TileMap Tiles.
	void drawTile(const std::string tile_map_id, const std::string& id, int x, int y, double angle = 0, int alpha = 255, bool centered = false, SDL_RendererFlip flip = SDL_FLIP_NONE);
	...
	bool tileMapExists(const std::string& tile_map_name);
...
private:
	...
	// Where the TileMaps are stored.
	std::unordered_map<std::string, TileMap*> m_tileMapMap;
